...
public class JaxbHelper {
	...
	final static Map<Class<?>, JaxbHelper> jaxbHelpers = new HashMap<Class<?>, JaxbHelper>();

	private static JaxbHelper getJaxbHelper(Class<?> clazz) throws JAXBException {
		synchronized (jaxbHelpers) {
			JaxbHelper jaxbHelper = jaxbHelpers.get(clazz);
			if (jaxbHelper == null) {
				jaxbHelper = new JaxbHelper(clazz, null);
				jaxbHelpers.put(clazz, jaxbHelper);
			}
			return jaxbHelper;
		}
	}
	...
	public static JaxbHelper get(Class<?> jaxbRootElement, List<Class<?>> extraClasses) {
		try {
			if (extraClasses == null || extraClasses.isEmpty()) {
				return getJaxbHelper(jaxbRootElement);
			} else {
	...
	public String marshal(Object object, boolean formatted) throws JAXBException {
		...
		return new String(baos.toByteArray(), Utf8.CHARSET);
	}
	...
	public static <T> T deserializeXmlObject(InputStream is, Class<T> clazz, boolean recordXml)
		...
		return deserializeXmlObject(new InputStreamReader(is, Utf8.CHARSET), clazz, recordXml);
	}
	...
	public <T> T deserialize(Reader reader, Class<T> clazz, boolean recordXml) throws UnmarshalException {
		...
		T value = Casts.as(jaxbObject, clazz);
		if (value == null) {

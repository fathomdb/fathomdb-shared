...
public class Extensions {
		...
		if (!Strings.isNullOrEmpty(extensionList)) {
			for (String extension : Splitter.on(',').split(extensionList)) {
				...
				try {
					...
					extensionClass = (Class<? extends ExtensionModule>) classLoader
							.loadClass(extension);
				} catch (ClassNotFoundException e) {
					throw new IllegalStateException(
							"Unable to load extension class: " + extension, e);
				}
				...
				try {
					...
					throw new IllegalStateException(
							"Unable to construct extension class: " + extension,
							e);
	...
	private void loadReflectedExtensions(Discovery discovery) {
		ServiceLoader<ExtensionModule> serviceLoader = ServiceLoader
				.load(ExtensionModule.class);
		...
		// Collection<Class> extensionTypes =
		// discovery.findAnnotatedClasses(Extension.class);
		// if (extensionTypes == null || extensionTypes.isEmpty()) {
		// log.info("No Annotated extensions found");
		// return;
		// }
		//
		// List<ExtensionModule> instances =
		// Discovery.buildInstances(ExtensionModule.class, extensionTypes);
	...
	public Injector createInjector(Configuration configuration,
			List<Module> modules) {
		for (ExtensionModule extension : extensions) {
			List<Module> extraModules = extension
					.getExtraModules(configuration);

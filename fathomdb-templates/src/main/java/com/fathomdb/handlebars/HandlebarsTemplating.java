...
public class HandlebarsTemplating {
	...
	// HandlebarsTemplating templating = (HandlebarsTemplating) servletContext.getAttribute(attributeKey);
	// if (templating == null) {
	// synchronized (servletContext) {
	// templating = (HandlebarsTemplating) servletContext.getAttribute(attributeKey);
	//
	// if (templating == null) {
	...
	public void writeTo(String resolvedPath, Object model, OutputStream out) throws IOException {
		...
		try {
			...
			throw new ContainerException("Error running template", e);
		}
	}
}
